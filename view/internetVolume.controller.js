...
*/
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for(var j in dc){
    			...
    			for (var i in sRes.results) {
    				...
    				if (sRes.results[i].KPI_TYPE == '利用小时'&&sRes.results[i].KPI_DESC==dc[j]){ 
    				    if (isFirst != true) {
    				        tempJsonStrData += ',';
    				    }
        			    tempJsonStrData += '"useHours":';
        			    tempJsonStrData += sRes.results[i].KPI_VALUE;
        			    isFirst = false;
    				}
    			}
	...
	//////////////////////////////////浙江省地图//////////////////////////////////////////////////////////		
				...
				myChart4.on(ecConfig.EVENT.CLICK, function (param){  
					...
					var data1 = mapSeries.markPoint.data[param.dataIndex].eachPlantPV;
					var data2 = mapSeries.markPoint.data[param.dataIndex].eachPlantPV - mapSeries.markPoint.data[param.dataIndex].factoryUsePV
				    
				    drawpie(ec, data1, data2, 'detail_pieNet');
				    drawbar(ec, data1, data2, 'detail_01Net');
				    drawbar(ec, data1, data2, 'detail_02Net');
				    drawbar(ec, data1, data2, 'detail_03Net');
				    drawbar(ec, data1, data2, 'detail_04Net');	
				    
				    // 发电量
				    document.getElementById('fuelCostNet').innerHTML = mapSeries.markPoint.data[param.dataIndex].eachPlantPV;
				    // 发电量同比变化
				    document.getElementById('fuelDownPercentNet').innerHTML = mapSeries.markPoint.data[param.dataIndex].powerPercentUp;
				    // 合约电量
				    document.getElementById('travelPriceNet').innerHTML = mapSeries.markPoint.data[param.dataIndex].contractPV;
				    // 直供电量
				    document.getElementById('coalPriceNet').innerHTML = mapSeries.markPoint.data[param.dataIndex].directlyPV;
				    // 厂用电量
				    document.getElementById('coalCostNet').innerHTML = mapSeries.markPoint.data[param.dataIndex].factoryUsePV;
				    // 替代电量
				    document.getElementById('watt1Net').innerHTML = mapSeries.markPoint.data[param.dataIndex].replacePV;
				    // 月利用小时
				    document.getElementById('watt2Net').innerHTML = mapSeries.markPoint.data[param.dataIndex].useHours;
				});	
		...
		///////////////////////////////安徽淮南市地图////////////////////////////////////////////
				...
				myChart5.on(ecConfig.EVENT.CLICK, function (param){
					...
					// 电厂名
					document.getElementById('powerPlantMainDetailTitleNet').innerHTML = '凤台电厂';
					var data1 = mapSeries.markPoint.data[param.dataIndex].eachPlantPV;
					var data2 = mapSeries.markPoint.data[param.dataIndex].eachPlantPV - mapSeries.markPoint.data[param.dataIndex].factoryUsePV
